...
public class MyCrashHandler implements UncaughtExceptionHandler {
	...
	private static final String TAG = "CrmCrashHandler";
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		...
		LogUtil.log(TAG, ex.getMessage());
		// 提示用户程序即将退出
		showToast(mContext, "Sorry, Exception happened!Please see the crash log！");
		try {
			Thread.sleep(1000);
	...
	private void writeCrashLog(Thread thread, Throwable ex) {
		if (mLogUtil == null)
			mLogUtil = new LogUtil(this.getClass(), thread.getName());
		mLogUtil.writeCrashLog(getErrInfo(ex));
	}
